<template>
  <div class="auth-stage" :class="themeClass">
    ...
    <main class="auth-center">
      ...
      <h1 class="auth-headline reveal" style="--d: 80ms">欢迎回来</h1>
      <p class="auth-tagline reveal" style="--d: 160ms">登录 Voice Factory 继续你的语音工作</p>
      ...
      <section class="auth-panel reveal" style="--d: 240ms">
        ...
        <p class="auth-test">测试账号 <code>admin / 123456</code></p>
      </section>
...
export default defineComponent({
  ...
  setup () {
    ...
    const handleLogin = async () => {
      ...
      loading.value = true
      try {
        const loginRes = await login(form.account.trim(), form.password.trim())
        if (loginRes.code !== 0) {
        ...
        store.commit('setAuthState', {
          ...
          user: {
            ...
            description: '已接入 springboot-minio 登录服务。',
            isLogin: true
          }
        })
        ...
        ElMessage.error(error?.response?.data?.message || '登录失败，请检查后端服务和数据库配置')
      } finally {
        loading.value = false
      }
    }

    return { themeClass, form, loading, handleLogin }
  }
})
</script>
