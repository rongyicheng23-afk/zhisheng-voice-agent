...
if [[ -f "$visible_env_file" ]]; then
  while IFS='=' read -r key raw_value; do
    ...
    [[ -z "$value" ]] && continue
    case "$key" in
      MYSQL_PASSWORD|REALTIME_GATEWAY_INTERNAL_KEY|DEEPSEEK_API_KEY|DEEPSEEK_MODEL|XFYUN_TTS_APP_ID|XFYUN_TTS_API_KEY|XFYUN_TTS_API_SECRET|XFYUN_TTS_VOICE|SPRING_BOOT_URL|SSL_CERT_FILE)
        export "$key=$value"
        ;;
    esac
