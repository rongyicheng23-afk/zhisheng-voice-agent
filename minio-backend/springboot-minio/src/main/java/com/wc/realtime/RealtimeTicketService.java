...
public class RealtimeTicketService {
    ...
    private record Entry(int userId, Instant expiresAt) {}
    public synchronized Issued issue(int userId) {
    ...
    public synchronized Integer consume(String ticket) {
        Entry entry = tickets.remove(ticket);
        LOG.info("event=ticket.consume accepted={}", entry != null && entry.expiresAt().isAfter(clock.instant()));
        return entry != null && entry.expiresAt().isAfter(clock.instant()) ? entry.userId() : null;
    }
}
